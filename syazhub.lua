--- LEAKED BY fwexternet ON DISCORD -- 
...
do
	...
	-- ENVY - vem da ESQUERDA para o CENTRO (mais em cima)
	local envyText = Instance.new("TextLabel")
	envyText.Size = UDim2.new(0, 320, 0, 130)
	envyText.Position = UDim2.new(-0.5, 0, 0.25, 0)
	envyText.BackgroundTransparency = 1
	envyText.Text = "syaz hub"
	envyText.TextColor3 = Color3.fromRGB(100, 100, 100)
	envyText.TextScaled = true
	envyText.Font = Enum.Font.GothamBlack
	envyText.TextStrokeTransparency = 0.5
	envyText.TextStrokeColor3 = Color3.new(0,0,0)
	envyText.TextTransparency = 1
	envyText.Parent = bg

	-- HUB - vem da DIREITA para o CENTRO (bem mais embaixo)
	local hubText = Instance.new("TextLabel")
	hubText.Size = UDim2.new(0, 270, 0, 110)
	hubText.Position = UDim2.new(1.5, 0, 0.58, 0)
	hubText.BackgroundTransparency = 1
	hubText.Text = "HUB"
	hubText.TextColor3 = Color3.fromRGB(140, 140, 140)
	hubText.TextScaled = true
	hubText.Font = Enum.Font.GothamBlack
	hubText.TextStrokeTransparency = 0.5
	hubText.TextStrokeColor3 = Color3.new(0,0,0)
	...
	-- ENVY vem da ESQUERDA para o CENTRO (posição superior)
	local envyTween = TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	local envyTarget = UDim2.new(0.35, 0, 0.25, 0)
	_TS:Create(envyText, envyTween, {Position = envyTarget, TextTransparency = 0}):Play()

	task.wait(0.35)

	-- HUB vem da DIREITA para o CENTRO (posição inferior)
	local hubTween = TweenInfo.new(0.7, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	local hubTarget = UDim2.new(0.35, 0, 0.58, 0)
	_TS:Create(hubText, hubTween, {Position = hubTarget, TextTransparency = 0}):Play()

	task.wait(3.5)

	local fadeOut = TweenInfo.new(1.1, Enum.EasingStyle.Quint)
	_TS:Create(bg, fadeOut, {BackgroundTransparency = 1}):Play()
	_TS:Create(envyText, fadeOut, {TextTransparency = 1}):Play()
...
local PLOTCACHEDURATION    = 2
local PROMPTCACHEREFRESH   = 0.15
...
local APLFACE=Vector3.new(-482.25,-4.96,92.09)
local AP_R1=Vector3.new(-476.16,-6.52,25.62)
local AP_R2=Vector3.new(-483.06,-5.03,25.48)
local APRFACE=Vector3.new(-482.06,-6.93,35.47)

-- ===== MOBILE PANEL DIMENSIONS (MAIOR) =====
local MBBTNW  = 90
local MBBTNH  = 75
local MB_COLS = 2
local MB_ROWS = 4
local MB_PAD  = 5
local MB_CORNER = 16
local MBTOTALW = MB_COLS * MBBTNW + (MB_COLS+1) * MB_PAD
local MBTOTALH = MB_ROWS * MBBTNH + (MB_ROWS+1) * MB_PAD
local mbPanel   = nil
...
local function isMyPlotByName(n)
	local ct=tick()
	if Steal.plotCache[n] and (ct-(Steal.plotCacheTime[n] or 0))<PLOTCACHEDURATION then return Steal.plotCache[n] end
...
local function findNearestPrompt()
	...
	local ct=tick()
	if ct-Steal.promptCacheTime<PROMPTCACHEREFRESH and Steal.cachedPrompts>0 then
...
-- ===================== INFINITE JUMP =====================
local IJJumpConn,IJFallConn=nil,nil
local function startInfiniteJump()
	if IJJumpConn then IJJumpConn:Disconnect() end
	if IJFallConn then IJFallConn:Disconnect() end
	IJJumpConn=UIS.JumpRequest:Connect(function()
	...
	IJFallConn=RunService.Heartbeat:Connect(function()
		if not infJumpEnabled then return end
		local char=LP.Character;if not char then return end
		local root=char:FindFirstChild("HumanoidRootPart")
...
local function stopInfiniteJump()
	if IJJumpConn then IJJumpConn:Disconnect();IJJumpConn=nil end
	if IJFallConn then IJFallConn:Disconnect();IJFallConn=nil end
end
	...
	alConn=RunService.Heartbeat:Connect(function()
		...
		elseif alPhase==2 then
			if (Vector3.new(AP_L2.X,hrp.Position.Y,AP_L2.Z)-hrp.Position).Magnitude<1 then
				...
				if (APLFACE-hrp.Position).Magnitude>0.01 then
					hrp.CFrame=CFrame.new(hrp.Position,Vector3.new(APLFACE.X,hrp.Position.Y,APLFACE.Z))
				end
				return
			end
			local d=AP_L2-hrp.Position;local mv=Vector3.new(d.X,0,d.Z).Unit
	...
	arConn=RunService.Heartbeat:Connect(function()
		...
		elseif arPhase==2 then
			if (Vector3.new(AP_R2.X,hrp.Position.Y,AP_R2.Z)-hrp.Position).Magnitude<1 then
				...
				if (APRFACE-hrp.Position).Magnitude>0.01 then
					hrp.CFrame=CFrame.new(hrp.Position,Vector3.new(APRFACE.X,hrp.Position.Y,APRFACE.Z))
				end
				return
			end
			local d=AP_R2-hrp.Position;local mv=Vector3.new(d.X,0,d.Z).Unit
...
local ACTIVETABBG  = ACCENT
local ACTIVETABTXT = WHITE
local IDLETABBG    = Color3.fromRGB(22,22,22)
local IDLETABTXT   = DIM
...
-- ALTURA REDUZIDA: 520 -> 400
local W,H,SW,CORNER=420,400,95,12
...
-- Main frame (com altura reduzida)
local main=Instance.new("Frame",gui)
main.Name="Main";main.Size=UDim2.new(0,W,0,H);main.Position=UDim2.new(1,-W-20,0,80)
...
topbar.Size=UDim2.new(1,0,0,45);topbar.BackgroundColor3=SIDEBAR;topbar.BorderSizePixel=0;topbar.ZIndex=10
...
titleLbl.Size=UDim2.new(0,180,1,0);titleLbl.Position=UDim2.new(0,15,0,0)
titleLbl.BackgroundTransparency=1;titleLbl.Text="ENVY HUB"
titleLbl.TextColor3=ACCENT;titleLbl.Font=Enum.Font.GothamBlack;titleLbl.TextSize=16
titleLbl.TextXAlignment=Enum.TextXAlignment.Left;titleLbl.ZIndex=12

local verLbl=Instance.new("TextLabel",topbar)
verLbl.Size=UDim2.new(0,120,1,0);verLbl.Position=UDim2.new(0,110,0,0)
verLbl.BackgroundTransparency=1;verLbl.Text="v1"
verLbl.TextColor3=DIM;verLbl.Font=Enum.Font.Gotham;verLbl.TextSize=9
verLbl.TextXAlignment=Enum.TextXAlignment.Left;verLbl.ZIndex=12

local creditLbl=Instance.new("TextLabel",topbar)
creditLbl.Size=UDim2.new(0,200,1,0);creditLbl.Position=UDim2.new(0,125,0,0)
creditLbl.BackgroundTransparency=1;creditLbl.Text="creds: Prime Idk"
creditLbl.TextColor3=DIM;creditLbl.Font=Enum.Font.Gotham;verLbl.TextSize=9
...
minBtn.Text="–";minBtn.TextColor3=WHITE;minBtn.Font=Enum.Font.GothamBlack;minBtn.TextSize=18;minBtn.ZIndex=13
...
sidebar.Size=UDim2.new(0,SW,1,-45-CORNER);sidebar.Position=UDim2.new(0,0,0,45)
...
local sidebarImage=Instance.new("ImageLabel",sidebar)
sidebarImage.Size=UDim2.new(1,0,0,120);sidebarImage.BackgroundTransparency=0
sidebarImage.BackgroundColor3=Color3.fromRGB(10,10,10)
sidebarImage.Image="rbxassetid://98443193472302"
sidebarImage.ImageColor3=Color3.fromRGB(255,255,255)
sidebarImage.ScaleType=Enum.ScaleType.Fit
sidebarImage.ZIndex=6
sidebarImage.LayoutOrder=1
Instance.new("UICorner",sidebarImage).CornerRadius=UDim.new(0,10)

local sideTopPatch=Instance.new("Frame",main)
sideTopPatch.Size=UDim2.new(0,SW,0,CORNER);sideTopPatch.Position=UDim2.new(0,0,0,45)
...
sideDiv.Size=UDim2.new(0,1,1,-45-CORNER);sideDiv.Position=UDim2.new(0,SW,0,45)
sideDiv.BackgroundColor3=DARK_ACC;sideDiv.BorderSizePixel=0;sideDiv.ZIndex=6

-- Content area
local content=Instance.new("Frame",main)
content.Name="ContentArea"
content.Size=UDim2.new(1,-SW-1,1,-45-CORNER);content.Position=UDim2.new(0,SW+1,0,45)
content.BackgroundColor3=BG;content.BorderSizePixel=0;content.ClipsDescendants=true;content.ZIndex=2

-- Mini button (maior)
local mini=Instance.new("TextButton",gui)
mini.Name="EnvyMini";mini.Size=UDim2.new(0,180,0,42);mini.Position=UDim2.new(1,-200,0,80)
mini.BackgroundColor3=SIDEBAR;mini.BorderSizePixel=0
mini.Text="ENVY HUB";mini.TextColor3=ACCENT
mini.Font=Enum.Font.GothamBlack;mini.TextSize=14;mini.ZIndex=20;mini.Visible=false
...
tabListFrame.Size=UDim2.new(1,0,0,0);tabListFrame.BackgroundTransparency=1;tabListFrame.BorderSizePixel=0;tabListFrame.ZIndex=6
tabListFrame.LayoutOrder=2
...
tabPad.PaddingTop=UDim.new(0,8);tabPad.PaddingLeft=UDim.new(0,5);tabPad.PaddingRight=UDim.new(0,5);tabPad.PaddingBottom=UDim.new(0,8)

local function switchTab(name)
	activeTabName=name
	for _,td in ipairs(tabDefs) do
		local t=tabs[td.name];local isA=td.name==name
		TweenService:Create(t.frame,TweenInfo.new(0.14),{BackgroundColor3=isA and ACTIVETABBG or IDLETABBG}):Play()
		TweenService:Create(t.lbl,TweenInfo.new(0.14),{TextColor3=isA and ACTIVETABTXT or IDLETABTXT}):Play()
...
for i,td in ipairs(tabDefs) do
	...
	btn.Size=UDim2.new(1,0,0,38);btn.BackgroundColor3=IDLETABBG;btn.BorderSizePixel=0
	...
	lbl.TextColor3=IDLETABTXT;lbl.Font=Enum.Font.GothamBold;lbl.TextSize=11
	...
	pll.SortOrder=Enum.SortOrder.LayoutOrder;pll.Padding=UDim.new(0,5)
	...
	btn.MouseLeave:Connect(function()
		if activeTabName~=td.name then TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=IDLETABBG}):Play() end
	end)
end

task.defer(function()
	tabListFrame.Size=UDim2.new(1,0,0,tabLL.AbsoluteContentSize.Y+16)
end)
...
local function makeSecHeader(tabName,text)
	local f=Instance.new("Frame",pg(tabName));f.Size=UDim2.new(1,0,0,22)
	f.BackgroundTransparency=1;f.BorderSizePixel=0;f.LayoutOrder=lo(tabName);f.ZIndex=4
	local t2=Instance.new("Frame",f);t2.Size=UDim2.new(0,4,0,12);t2.Position=UDim2.new(0,0,0.5,-6)
	t2.BackgroundColor3=ACCENT;t2.BorderSizePixel=0;Instance.new("UICorner",t2).CornerRadius=UDim.new(0,2)
	local lbl=Instance.new("TextLabel",f);lbl.Size=UDim2.new(1,-10,1,0);lbl.Position=UDim2.new(0,10,0,0)
	lbl.BackgroundTransparency=1;lbl.Text=text:upper();lbl.TextColor3=ACCENT
	lbl.Font=Enum.Font.GothamBold;lbl.TextSize=10;lbl.TextXAlignment=Enum.TextXAlignment.Left;lbl.ZIndex=5
end

local function baseCard(tabName,h2)
	local c=Instance.new("Frame",pg(tabName))
	c.Size=UDim2.new(1,0,0,h2 or 42);c.BackgroundColor3=CARD_BG;c.BorderSizePixel=0
...
local function cLabel(p,text,x,w,sz,col,font,xa)
	...
	l.Size=UDim2.new(0,w or 150,1,0);l.Position=UDim2.new(0,x or 10,0,0)
	l.BackgroundTransparency=1;l.Text=text;l.TextColor3=col or WHITE
	l.Font=font or Enum.Font.GothamBold;l.TextSize=sz or 11
...
local function makePillToggle(parent,defOn,onToggle)
	...
	pbg.Size=UDim2.new(0,PW,0,PH);pbg.Position=UDim2.new(1,-(PW+10),0.5,-PH/2)
	...
	dot.Position=defOn and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7)
...
local function makeKB(parent,kbEntry,onChange)
	...
	b.Size=UDim2.new(0,48,0,22);b.BackgroundColor3=KB_BG;b.BorderSizePixel=0
	local function getDisplayText()
		if kbEntry.gp then return "🎮"..kbEntry.gp.Name end
		return (kbEntry.kb or Enum.KeyCode.Unknown).Name
	end
	b.Text=getDisplayText()
	b.TextColor3=WHITE;b.Font=Enum.Font.GothamBold;b.TextSize=9;b.ZIndex=11
...
local function rowToggle(tabName,label,sub,defOn,onToggle)
	local c=baseCard(tabName,sub and 52 or 42)
	cLabel(c,label,10,160,11,WHITE,Enum.Font.GothamBold)
	if sub then
		local sl=cLabel(c,sub,10,180,9,DIM,Enum.Font.Gotham)
		sl.Size=UDim2.new(0,180,0,14);sl.Position=UDim2.new(0,10,0,26)
	end
...
local function rowToggleKB(tabName,label,sub,kbEntry,defOn,onToggle,onKeyChange)
	...
	if sub then
		...
		sl.Size=UDim2.new(0,160,0,14);sl.Position=UDim2.new(0,10,0,26)
	end
	local kb=makeKB(c,kbEntry,function(k)
		kbEntry.kb=k;kbEntry.gp=nil;if onKeyChange then onKeyChange(k) end
	end)
	kb.Position=UDim2.new(1,-(48+10+40+8),0.5,-11);kb.ZIndex=11
	local PW,PH=38,20
	...
	Instance.new("UICorner",pbg).CornerRadius=UDim.new(0,10)
	local ps=Instance.new("UIStroke",pbg);ps.Color=defOn and DARK_ACC or BORDER2;ps.Thickness=1
	local dot=Instance.new("Frame",pbg);dot.Size=UDim2.new(0,14,0,14)
	dot.Position=defOn and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7)
	dot.BackgroundColor3=WHITE;dot.BorderSizePixel=0;dot.ZIndex=9
	Instance.new("UICorner",dot).CornerRadius=UDim.new(0,4)
	local isOn=defOn or false
	local function setV(on)
		...
		TweenService:Create(ps,TweenInfo.new(0.18),{Color=on and DARK_ACC or BORDER2}):Play()
		TweenService:Create(dot,TweenInfo.new(0.18,Enum.EasingStyle.Back),{
			Position=on and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7),BackgroundColor3=WHITE
		}):Play()
	end
...
local function rowKBOnly(tabName,label,sub,kbEntry,onKeyChange)
	local c=baseCard(tabName,sub and 52 or 42)
	cLabel(c,label,10,170,11,WHITE,Enum.Font.GothamBold)
	if sub then
		local sl=cLabel(c,sub,10,190,9,DIM,Enum.Font.Gotham)
		sl.Size=UDim2.new(0,190,0,14);sl.Position=UDim2.new(0,10,0,26)
	end
	local kb=makeKB(c,kbEntry,function(k)
		kbEntry.kb=k;kbEntry.gp=nil;if onKeyChange then onKeyChange(k) end
	end)
	kb.Position=UDim2.new(1,-(48+10),0.5,-11);kb.ZIndex=11
	return kb
end

local function rowInput(tabName,label,sub,default,onChange)
	local c=baseCard(tabName,sub and 52 or 42)
	cLabel(c,label,10,140,11,WHITE,Enum.Font.GothamBold)
	if sub then
		local sl=cLabel(c,sub,10,170,9,DIM,Enum.Font.Gotham)
		sl.Size=UDim2.new(0,170,0,14);sl.Position=UDim2.new(0,10,0,26)
	end
	...
	box.TextColor3=WHITE;box.Font=Enum.Font.GothamBold;box.TextSize=11
...
local function rowActionBtnWhite(tabName,label,onClick)
	...
	b.Size=UDim2.new(1,0,0,45);b.BackgroundColor3=Color3.fromRGB(16,16,16);b.BorderSizePixel=0
	b.Text=label;b.TextColor3=Color3.fromRGB(235,235,235);b.Font=Enum.Font.GothamBlack;b.TextSize=12
...
pbFrame.Size=UDim2.new(0,280,0,52);pbFrame.Position=UDim2.new(0,20,1,-65)
...
mbPanel.Position         = UDim2.new(1, -(MBTOTALW+15), 0.5, -(MBTOTALH/2))
...
makeDraggable(mbPanel)

local function makeMobileBtn(label, row, col, onClick)
	local x = MB_PAD + (col-1)*(MBBTNW+MB_PAD)
	local y = MB_PAD + (row-1)*(MBBTNH+MB_PAD)
	local btn = Instance.new("TextButton", mbPanel)
	btn.Size             = UDim2.new(0, MBBTNW, 0, MBBTNH)
	...
	btn.MouseButton1Down:Connect(function()
		TweenService:Create(btn, TweenInfo.new(0.05), {BackgroundColor3=Color3.fromRGB(25,25,25)}):Play()
	end)
...
do
	local c=baseCard("Speed",42)
	cLabel(c,"Mode",10,80,11,WHITE,Enum.Font.GothamBold)
	modeValLbl=cLabel(c,"Normal",100,80,10,DIM,Enum.Font.GothamBold,Enum.TextXAlignment.Left)
...
do
	local c=baseCard("Mechanics",52)
	cLabel(c,"Steal Duration",10,170,11,WHITE,Enum.Font.GothamBold)
	local sl=cLabel(c,"Prompt hold time (sec)",10,190,9,DIM,Enum.Font.Gotham)
	sl.Size=UDim2.new(0,190,0,14);sl.Position=UDim2.new(0,10,0,26)
	local box=Instance.new("TextBox",c)
	box.Size=UDim2.new(0,70,0,26);box.Position=UDim2.new(1,-80,0.5,-13)
...
do
	local c=baseCard("Mechanics",42)
	cLabel(c,"Grab Radius",10,130,11,WHITE,Enum.Font.GothamBold)
	local radValBtn=Instance.new("TextButton",c)
	radValBtn.Size=UDim2.new(0,70,0,26);radValBtn.Position=UDim2.new(1,-80,0.5,-13)
	...
	radValBtn.TextSize=11;radValBtn.ZIndex=11
...
do
	local c=baseCard("Movement",42)
	cLabel(c,"Float",10,130,11,WHITE,Enum.Font.GothamBold)
...
do
	local c=baseCard("Settings",42)
	cLabel(c,"Lock UI",10,140,11,WHITE,Enum.Font.GothamBold)
	local PW,PH=38,20
	local pbg=Instance.new("Frame",c)
	pbg.Size=UDim2.new(0,PW,0,PH);pbg.Position=UDim2.new(1,-(PW+10),0.5,-PH/2)
	pbg.BackgroundColor3=OFF_BG;pbg.BorderSizePixel=0;pbg.ZIndex=8
	...
	dot.BackgroundColor3=WHITE;dot.BorderSizePixel=0;dot.ZIndex=9
	Instance.new("UICorner",dot).CornerRadius=UDim.new(0,4)
	local sublbl=cLabel(c,"Unlocked",10+140+8,70,9,DIM,Enum.Font.Gotham)
	local clk=Instance.new("TextButton",c)
	clk.Size=UDim2.new(1,0,1,0);clk.BackgroundTransparency=1;clk.Text="";clk.ZIndex=6
	clk.MouseButton1Click:Connect(function()
		if _anyKeyListening then return end
		guiLocked=not guiLocked
		local unlocked=not guiLocked
		...
		TweenService:Create(dot,TweenInfo.new(0.18,Enum.EasingStyle.Back),{
			Position=unlocked and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7),BackgroundColor3=WHITE
		}):Play()
...
do
	local c=baseCard("Settings",52)
	cLabel(c,"Button Size",10,160,11,WHITE,Enum.Font.GothamBold)
	local sub=cLabel(c,"Panel size – tap +/- to resize",10,210,9,DIM,Enum.Font.Gotham)
	sub.Size=UDim2.new(0,210,0,14);sub.Position=UDim2.new(0,10,0,26)
	local valLbl=Instance.new("TextLabel",c)
	valLbl.Size=UDim2.new(0,40,0,18);valLbl.Position=UDim2.new(1,-(40+10),0.5,-9)
	valLbl.BackgroundTransparency=1;valLbl.Text=tostring(MBBTNW).."px"
	valLbl.TextColor3=ACCENT;valLbl.Font=Enum.Font.GothamBold;valLbl.TextSize=10
	valLbl.TextXAlignment=Enum.TextXAlignment.Right;valLbl.ZIndex=10
	local decBtn=Instance.new("TextButton",c)
	decBtn.Size=UDim2.new(0,24,0,24);decBtn.Position=UDim2.new(1,-(24+40+10+10),0.5,-12)
	decBtn.BackgroundColor3=Color3.fromRGB(16,16,16);decBtn.BorderSizePixel=0
	decBtn.Text="-";decBtn.TextColor3=WHITE;decBtn.Font=Enum.Font.GothamBlack;decBtn.TextSize=15;decBtn.ZIndex=11
	...
	incBtn.Size=UDim2.new(0,24,0,24);incBtn.Position=UDim2.new(1,-(24+10),0.5,-12)
	incBtn.BackgroundColor3=Color3.fromRGB(16,16,16);incBtn.BorderSizePixel=0
	incBtn.Text="+";incBtn.TextColor3=WHITE;incBtn.Font=Enum.Font.GothamBlack;incBtn.TextSize=15;incBtn.ZIndex=11
	...
	local function applyBtnSize(w,h)
		...
		MBBTNW=w;MBBTNH=h
		MBTOTALW=MB_COLS*MBBTNW+(MB_COLS+1)*MB_PAD
		MBTOTALH=MB_ROWS*MBBTNH+(MB_ROWS+1)*MB_PAD
		mbPanel.Size=UDim2.new(0,MBTOTALW,0,MBTOTALH)
		valLbl.Text=tostring(MBBTNW).."px"
		local idx=0
		for _,child in ipairs(mbPanel:GetChildren()) do
			if child:IsA("TextButton") then
				...
				local x=MB_PAD+(col-1)*(MBBTNW+MB_PAD)
				local y=MB_PAD+(row-1)*(MBBTNH+MB_PAD)
				child.Size=UDim2.new(0,MBBTNW,0,MBBTNH)
	...
	decBtn.MouseButton1Click:Connect(function()
		if _anyKeyListening then return end
		applyBtnSize(MBBTNW-4, MBBTNH-3)
	end)
...
do
	...
	b.TextColor3=Color3.fromRGB(235,235,235);b.Font=Enum.Font.GothamBlack;b.TextSize=12
	...
	b.MouseLeave:Connect(function() TweenService:Create(b,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(16,16,16)}):Play() end)
	b.MouseButton1Click:Connect(function()
		main.Position=UDim2.new(1,-W-20,0,80)
		mini.Position=UDim2.new(1,-200,0,80)
		pbFrame.Position=UDim2.new(0,20,1,-65)
		mbPanel.Position=UDim2.new(1,-(MBTOTALW+15),0.5,-(MBTOTALH/2))
	end)
end
...
task.spawn(function()
    while task.wait() do
        BorderGradient.Rotation = (BorderGradient.Rotation + 0.8) % 360
    end
end)

-- Title: "syaz hub" with animated gradient color
local TitleLabel = Instance.new("TextLabel")
TitleLabel.TextXAlignment = Enum.TextXAlignment.Left
TitleLabel.Font = Enum.Font.GothamBlack
TitleLabel.BackgroundTransparency = 1
TitleLabel.Position = UDim2.new(0, 12, 0, 6)
TitleLabel.TextColor3 = Color3.fromRGB(60, 130, 255)
TitleLabel.Text = "AEZY Lagger"
...
TitleGradient.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.33, Color3.fromRGB(120, 220, 255)),
...
SubLabel.Text = "Lagger"
SubLabel.Position = UDim2.new(0, 12, 0, 25)
SubLabel.TextSize = 11
SubLabel.Size = UDim2.new(1, -45, 0, 15)
SubLabel.Parent = Frame

-- Minimize button (blue)
local MinimizeBtn = Instance.new("TextButton")
MinimizeBtn.Font = Enum.Font.GothamBlack
MinimizeBtn.BackgroundColor3 = Color3.fromRGB(50, 100, 230)
...
MinStroke.Color = Color3.fromRGB(80, 150, 255)
MinStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
MinStroke.Parent = MinimizeBtn

-- Divider line
local Divider = Instance.new("Frame")
Divider.BackgroundColor3 = Color3.fromRGB(50, 80, 180)
...
ToggleRow.Position = UDim2.new(0, 10, 0, 52)
ToggleRow.BackgroundColor3 = Color3.fromRGB(25, 25, 40)
...
local ToggleRowStroke = Instance.new("UIStroke")
ToggleRowStroke.Thickness = 1.5
ToggleRowStroke.Color = Color3.fromRGB(50, 100, 220)
ToggleRowStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
ToggleRowStroke.Parent = ToggleRow
...
SwitchBgStroke.Thickness = 1.5
SwitchBgStroke.Color = Color3.fromRGB(80, 130, 255)
...
KeybindRow.Position = UDim2.new(0, 10, 0, 96)
KeybindRow.BackgroundColor3 = Color3.fromRGB(25, 25, 40)
KeybindRow.Size = UDim2.new(1, -20, 0, 36)
...
local KeybindRowStroke = Instance.new("UIStroke")
KeybindRowStroke.Thickness = 1.5
KeybindRowStroke.Color = Color3.fromRGB(50, 100, 220)
KeybindRowStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
KeybindRowStroke.Parent = KeybindRow
...
KeybindBtnStroke.Thickness = 1.5
KeybindBtnStroke.Color = Color3.fromRGB(100, 160, 255)
...
local originalSize = UDim2.new(0, 220, 0, 145)
local minimizedSize = UDim2.new(0, 220, 0, 44)
...
UserInputService.InputChanged:Connect(function(input)
    if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        if minimized then dragging = false return end
...
local function setToggle(state)
    ...
    local color = state and Color3.fromRGB(50, 110, 240) or Color3.fromRGB(45, 45, 65)
