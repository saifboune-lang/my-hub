...
-- ============================================================
local State = {
    normalSpeed=60.2, carrySpeed=29, laggerSpeed=11,
...
local Steal = {
    AutoStealEnabled=false, StealRadius=9, StealDuration=0.25,
...
-- ============================================================
local Presets = {}
local PRESET_FILE = "SYAZ.json"
local LAST_PRESET_FILE = "SYAZ.1.json"
local CONFIG_FILE = "SYAZ.json"
...
local C = {
    ...
    winBorder   = Color3.fromRGB(35, 35, 35),
    topBg       = Color3.fromRGB(15, 15, 15),
    topTitle    = Color3.fromRGB(240,240,240),
    topSub      = Color3.fromRGB(80, 80, 80),
    topBtn      = Color3.fromRGB(60, 60, 60),
    topBtnHov   = Color3.fromRGB(90, 90, 90),
    topDivider  = Color3.fromRGB(30, 30, 30),
    tabBarBg    = Color3.fromRGB(18, 18, 18),
    tabBarDiv   = Color3.fromRGB(30, 30, 30),
    tabIdle     = Color3.fromRGB(100,100,100),
    tabActive   = Color3.fromRGB(230,230,230),
    tabActiveBg = Color3.fromRGB(26, 26, 26),
    tabUnderline= Color3.fromRGB(200,200,200),
    sectionTxt  = Color3.fromRGB(90, 90, 90),
    sectionDiv  = Color3.fromRGB(28, 28, 28),
    rowBg       = Color3.fromRGB(0,  0,  0),
    rowBorder   = Color3.fromRGB(28, 28, 28),
    rowLabel    = Color3.fromRGB(210,210,210),
    rowSub      = Color3.fromRGB(85, 85, 85),
    rowValue    = Color3.fromRGB(170,170,170),
    rowHov      = Color3.fromRGB(22, 22, 22),
    inputBg     = Color3.fromRGB(22, 22, 22),
    inputBorder = Color3.fromRGB(40, 40, 40),
    inputFocus  = Color3.fromRGB(120,120,120),
    inputTxt    = Color3.fromRGB(220,220,220),
    pillOff     = Color3.fromRGB(28, 28, 28),
    ...
    modeBtnBg   = Color3.fromRGB(22, 22, 22),
    ...
    chipBg      = Color3.fromRGB(22, 22, 22),
    chipBorder  = Color3.fromRGB(38, 38, 38),
    chipTxt     = Color3.fromRGB(100,100,100),
    btnBg       = Color3.fromRGB(28, 28, 28),
    btnBorder   = Color3.fromRGB(44, 44, 44),
    btnTxt      = Color3.fromRGB(180,180,180),
    btnHov      = Color3.fromRGB(40, 40, 40),
    stackBg     = Color3.fromRGB(14, 14, 14),
    stackBrd    = Color3.fromRGB(28, 28, 28),
    stackTxt    = Color3.fromRGB(90, 90, 90),
    stackActBg  = Color3.fromRGB(28, 28, 28),
    stackActBrd = Color3.fromRGB(110,110,110),
    stackActTxt = Color3.fromRGB(210,210,210),
    stackDot    = Color3.fromRGB(35, 35, 35),
    stackDotOn  = Color3.fromRGB(150,150,150),
    infoBg      = Color3.fromRGB(10, 10, 10),
    infoBrd     = Color3.fromRGB(28, 28, 28),
    ...
    lockOn      = Color3.fromRGB(180,180,180),
...
-- CLEANUP
-- ============================================================
for _,name in pairs({"VyseSlottedGUI","VyseAsireGUI","VyseAsireHubV4","VyseAsireHubV5","VyseAsireHubV5_1","AsireHubV5_1","AsireHubV5_2","OpiumGGV5_2"}) do
...
gui.Name="OpiumGGV5_2"; gui.ResetOnSpawn=false; gui.DisplayOrder=10
...
buildPage("Movement", function()
    ...
    local function setDirActive(active)
        for _, dname in ipairs({"Left","Right"}) do
            ...
            TweenService:Create(b, TweenInfo.new(0.15), {
                BackgroundColor3 = isA and C.modeBtnActBg or Color3.fromRGB(0,0,0),
                BackgroundTransparency = isA and 0 or 1,
                TextColor3 = isA and C.modeBtnActTx or C.modeBtnTxt,
            }):Play()
        end
        duelDir = active:lower()
        -- Update watcher direction if already running
...
buildPage("Settings", function()
    ...
    fl.BackgroundTransparency = 1; fl.Text = "opium.gg  ·  v5.2"
...
vBtnFrame.Name = "OpiumGGVBtn"; vBtnFrame.Size = UDim2.new(0, 36, 0, 36)
...
infoBar.Size = UDim2.new(0, 210, 0, 56)
...
local function makeStatMini(xOff, w, icon)
    ...
    local iL = Instance.new("TextLabel", box); iL.Size = UDim2.new(0, 26, 1, 0)
    iL.BackgroundTransparency = 1; iL.Text = icon; iL.TextColor3 = C.infoTxt
    iL.Font = Enum.Font.GothamBold; iL.TextSize = 9
    local vL = Instance.new("TextLabel", box); vL.Size = UDim2.new(1, -26, 1, 0); vL.Position = UDim2.new(0, 26, 0, 0)
...
local fpsVal = makeStatMini(12, 56, "FPS"); local pingVal = makeStatMini(70, 64, "PING")
...
for i, def in ipairs(stackDefs) do
    ...
    mkCorner(btnFrame, 8); local bStroke = mkStroke(btnFrame, C.stackBrd, 1)
...
-- Watches for the duel countdown GUI to disappear, then fires
-- whichever of autoLeft / autoRight was queued.
-- ============================================================
...
local function startDuelCountdownWatcher(direction)
    ...
    -- Helper: is a countdown GUI visible right now?
    local function countdownVisible()
        local pg = LP:FindFirstChild("PlayerGui"); if not pg then return false end
        -- Murder Mystery 2 duel countdown typically lives in a ScreenGui with "Duel" or "Countdown" in the name
        -- We also check for any TextLabel showing "3", "2", "1" near screen center
    ...
    -- Wait until countdown appears, then wait for it to disappear, then trigger
    ...
    Conns_duelWatch = RunService.Heartbeat:Connect(function()
        ...
        if sawCountdown and not visible then
            ...
            -- Re-arm watcher so it fires again next duel
...
local function executeSteal(prompt)
    ...
    Conns.progress=RunService.Heartbeat:Connect(function() if not State.isStealing then Conns.progress:Disconnect(); return end; local prog=math.clamp((tick()-State.stealStartTime)/Steal.StealDuration,0,1); progressFill.Size=UDim2.new(prog,0,1,0); stealPctLbl.Text=math.floor(prog*100).."%" end)
    task.spawn(function()
...
loadConfig = function()
    ...
    if not hasFile then pcall(function() hasFile = _isfile("OpiumGGConfig.json") end) end
    if not hasFile then return end

    local raw
    local ok = pcall(function() raw = _readfile(CONFIG_FILE) end)
    if not ok or not raw then pcall(function() raw = _readfile("OpiumGGConfig.json") end) end
    if not raw then return end
...
local function setupChar(char)
    ...
    if head then
        local oldBB=head:FindFirstChild("OpiumGGBB"); if oldBB then oldBB:Destroy() end
...
RunService.RenderStepped:Connect(function()
    ...
    pcall(function()
        ...
        if head2 then
            local bb2 = head2:FindFirstChild("OpiumGGBB")
    ...
    if lastPresetName and lastPresetName ~= "" then
        for _, preset in ipairs(Presets) do
            if preset.name == lastPresetName then
                ...
                print("[OpiumGG v5.2] Auto-loaded last preset: " .. lastPresetName)
                break
            end
        end
    end
